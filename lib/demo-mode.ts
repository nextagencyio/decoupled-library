...
import branchesData from '@/data/mock/branches.json'
...
export function handleMockQuery(body: string): any {
  try {
    ...
    if (query.includes('GetBranches') || query.includes('nodeBranches')) {
      return loadMockData('branches.json')
    }
